...
/**
 * Thrown when a WayaQuick API request fails — a non-2xx HTTP status, an envelope with
 ...
 */
public class WayaQuickException extends RuntimeException {

    /** HTTP status code associated with the failure, or 0 when the request never got a response. */
    private final int statusCode;

    public WayaQuickException(String message) {
        this(message, 0, null);
    }

    public WayaQuickException(String message, int statusCode) {
        this(message, statusCode, null);
    }

    public WayaQuickException(String message, int statusCode, Throwable cause) {
        super(message, cause);
