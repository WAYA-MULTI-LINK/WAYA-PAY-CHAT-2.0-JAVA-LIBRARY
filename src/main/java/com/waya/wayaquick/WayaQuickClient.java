...
import com.waya.wayapay.model.WayaPayResponse;
import com.waya.wayapay.service.Collection;
import com.waya.wayapay.service.Identity;
import com.waya.wayapay.service.Payouts;
import com.waya.wayapay.service.Webhooks;
 ...
 */
public final class WayaPayClient {

    private static final SecureRandom RANDOM = new SecureRandom();

    private final WayaPayOptions options;
    ...
    public WayaPayClient(String merchantId, String secretKey) {
        this(WayaPayOptions.builder().merchantId(merchantId).secretKey(secretKey).build());
    }
     ...
     * @throws WayaPayException on a non-2xx status, {@code success == false}, network error, or timeout
     */
    public <T> T request(String method, String path, Object body, Map<String, String> query, JavaType dataType) {
        ...
        JavaType envelopeType = mapper.getTypeFactory()
                .constructParametricType(WayaPayResponse.class, dataType);
        ...
        while (true) {
            ...
            try {
                ...
                if (retryable && attempt < ceiling) { attempt++; backoff(attempt); continue; }
                throw new WayaPayException("Network error on " + path + ": " + e.getMessage(), 0, e);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new WayaPayException("Request to " + path + " was interrupted.", 0, e);
            }
            ...
            if (failed) {
                ...
                throw new WayaPayException(message, status);
            }
    ...
    private String serialize(Object body, String path) {
        try {
            ...
            throw new WayaPayException("Failed to serialise request body for " + path + ": " + e.getMessage(), 0, e);
        }
    }
    ...
    private void backoff(int attempt) {
        ...
        try {
            ...
            Thread.currentThread().interrupt();
            throw new WayaPayException("Retry backoff was interrupted.", 0, e);
        }
    }
}
