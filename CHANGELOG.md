...
- `client.webhooks()` and the static `WayaQuickWebhook` — HMAC-SHA256 signature verification, 5-minute replay window, and parsing into a `WebhookEvent` (constant-time signature comparison)
- `WayaQuickClient.generateReference(prefix)` — timestamped, collision-resistant idempotency key (`PREFIX-millis-HEX`)
...
- `HttpClient` injection via `WayaQuickOptions.builder().httpClient(...)` for connection pooling, proxies, and test fakes
- Builder-based configuration (`WayaQuickOptions`) and request DTOs (`PayoutRequest`, `CollectionRequest`)
...
- API failures throw `WayaQuickException` (carrying `statusCode()`); input validation throws `IllegalArgumentException` before any network call
